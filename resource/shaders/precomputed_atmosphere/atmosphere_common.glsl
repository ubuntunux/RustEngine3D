...
layout(binding = 6) uniform sampler2D texture_normal;
layout(binding = 7) uniform sampler3D texture_cloud;
layout(binding = 8) uniform sampler3D texture_noise;
...
void GetRMuSFromIrradianceTextureUv(const in AtmosphereParameters atmosphere, const in vec2 uv, out float r, out float mu_s)
{
...
vec3 GetIrradiance(const in AtmosphereParameters atmosphere, const in sampler2D irradiance_texture, float r, float mu_s)
{
...
{
    return ray_r_mu_intersects_ground ? DistanceToBottomAtmosphereBoundary(atmosphere, r, mu) : DistanceToTopAtmosphereBoundary(atmosphere, r, mu);
}
...
vec3 ComputeDirectIrradiancetexture2D(const in AtmosphereParameters atmosphere, const in sampler2D transmittance_texture, const in vec2 frag_coord)
{
    float r;
    float mu_s;
...
vec3 GetSolarRadiance(const in AtmosphereParameters atmosphere)
{
    return atmosphere.solar_irradiance /
        ...
        atmosphere_constants.SUN_RADIANCE_TO_LUMINANCE *
        atmosphere_constants.atmosphere_exposure;
}
...
)
{
    return ComputeSkyRadiance(
        ...
        sun_direction,
        transmittance
    ) * atmosphere_constants.SKY_RADIANCE_TO_LUMINANCE;
}
...
{
    return ComputeSkyRadianceToPoint(
        atmosphere,
        transmittance_texture,
        scattering_texture,
        single_mie_scattering_texture,
...
)
{
    ...
    sky_irradiance *= atmosphere_constants.SKY_RADIANCE_TO_LUMINANCE;
    return sun_irradiance * atmosphere_constants.SUN_RADIANCE_TO_LUMINANCE;
}
...
float GetSunVisibility(vec3 point, vec3 sun_direction)
{
    ...
    if (distance_to_intersection > 0.0)
    {
        ...
        return smoothstep(1.0, 0.0, ray_sphere_angular_distance / atmosphere_constants.sun_size.x);
    }
    return 1.0;
}
...
{
    vec3 pos = view_constants.CAMERA_POSITION.xyz * ATMOSPHERE_RATIO - kSphereCenter;
    float pos_dot_sun = dot(pos, sun_direction);
    float view_dot_sun = dot(view_direction, sun_direction);
    float k = atmosphere_constants.sun_size.x;
    float l = 1.0 + k * k;
...
float GetSceneShadowLength(float scene_dist, vec3 view_direction, sampler2D texture_shadow)
{
    ...
    const int LOOP = 64;
    float d = min(view_constants.NEAR_FAR.y * 0.01, scene_dist) / float(LOOP);
    float enter_count = 0.0;

    for(int i=0; i<LOOP; ++i)
    {
        float ray_dist = float(i) * d;
        vec3 world_pos = view_constants.CAMERA_POSITION.xyz + view_direction * ray_dist;
        vec4 shadow_uv = light_constants.SHADOW_VIEW_PROJECTION * vec4(world_pos, 1.0);
    ...
    vec3 relative_camera_pos = view_constants.CAMERA_POSITION.xyz * ATMOSPHERE_RATIO;
    float lightshaft_fadein_hack = smoothstep(0.02, 0.04, dot(normalize(relative_camera_pos - atmosphere_constants.earth_center), sun_direction));
...
)
{
    ...
    sun_irradiance = GetSunAndSkyIrradiance(atmosphere, relative_point.xyz - atmosphere_constants.earth_center, normal, sun_direction, sky_irradiance);

    vec3 transmittance;
    in_scatter = GetSkyRadianceToPoint(
        atmosphere,
        relative_camera_pos - atmosphere_constants.earth_center,
...
{
    vec3 sun_direction = light_constants.LIGHT_DIRECTION.xyz;
    vec3 camera_pos = view_constants.CAMERA_POSITION.xyz * ATMOSPHERE_RATIO;
    vec3 point = camera_pos + eye_direction.xyz * max(view_constants.NEAR_FAR.x, dist) * ATMOSPHERE_RATIO;

    sun_irradiance = GetSunAndSkyIrradiance(atmosphere, point.xyz - atmosphere_constants.earth_center, sun_direction, sun_direction, sky_irradiance);

    vec3 transmittance;
    inscatter = GetSkyRadianceToPoint(
        atmosphere,
        camera_pos - atmosphere_constants.earth_center,
