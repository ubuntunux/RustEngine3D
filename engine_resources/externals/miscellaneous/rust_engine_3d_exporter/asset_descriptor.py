...
class BaseDescriptor:
    ...
    def process(self):
        ...
        self._assets = {}
        for asset_path_name in self._asset_path_names:
            ...
            for ext in self._exts:
                for filepath in asset_path.rglob(f'*{ext}'):
                    ...
                    self.register_asset(self._asset_class(asset_name.as_posix(), filepath))
        self._logger.info(f'{self._asset_class._asset_type}: {len(self._assets)}')

class MaterialDescriptor(BaseDescriptor):
    _asset_class = MaterialInfo
    _asset_path_names = ['Materials']
    _exts = ['.mat']
...
class TextureDescriptor(BaseDescriptor):
    _asset_class = TextureInfo
    _asset_path_names = ['Terrain', 'Textures']
...
class AssetDescriptor:
    def __init__(self, logger, root_path):
        ...
        self._material_descriptor = MaterialDescriptor(self, logger)
        self._mesh_descriptor = MeshDescriptor(self, logger)
        self._model_descriptor = ModelDescriptor(self, logger)
        self._scene_descriptor = SceneDescriptor(self, logger)
    ...
    def process(self):
        ...
        self._material_descriptor.process()
        self._mesh_descriptor.process()
        self._model_descriptor.process()
        self._scene_descriptor.process()
