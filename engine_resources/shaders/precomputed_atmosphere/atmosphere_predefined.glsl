#ifndef _ATMOSPHERE_PREDEFINED_
#define _ATMOSPHERE_PREDEFINED_
...
float GetSunVisibility(const in ATMOSPHERE_CONSTANTS atmosphere_constants, vec3 point, vec3 sun_direction)
{
...
float GetSceneShadowLength(
    const in ATMOSPHERE_CONSTANTS atmosphere_constants,
    float scene_dist,
    float far,
    const in vec3 camera_position,
    const in vec3 view_direction,
    const in vec3 light_direction,
...
)
{
    ...
    return max(0.0, shadow_length * ATMOSPHERE_RATIO) * lightshaft_fadein_hack;
}
...
)
{
    ...
    scene_shadow_length = GetSceneShadowLength(
        atmosphere_constants,
        scene_linear_depth,
        far_dist,
        camera_position,
...
)
{
    ...
    vec3 point = camera_pos + eye_direction.xyz * max(near_dist, dist) * ATMOSPHERE_RATIO;
