...
pub trait SceneManagerBase {
    fn initialize_scene_manager_data(&mut self, renderer_data: &RendererData, resources: &Resources);
    fn regist_scene_graphics_data(&mut self, renderer_data: &RendererData, resources: &Resources);
    fn initialize_scene_graphics_data(&mut self, renderer_data: &RendererData, resources: &Resources);
    fn destroy_scene_graphics_data(&mut self, device: &Device);
...
impl SceneManagerData {
    pub fn create_scene_manager_data(
        ...
        scene_manager.borrow_mut().initialize_scene_manager_data(&renderer_data.borrow(), &resources.borrow());
        scene_manager_data
    }
    ...
    pub fn regist_scene_graphics_data(&self) {
        self._scene_manager.borrow_mut().regist_scene_graphics_data(&self._renderer_data.borrow(), &self._resources.borrow());
    }
    ...
    pub fn resized_window(&self, width: u32, height: u32) {
        self._scene_manager.borrow().resized_window(width, height);
    }
