...
pub fn device_create_info_set_push_next<T: vk::ExtendsDeviceCreateInfo>(
    create_info: &mut vk::DeviceCreateInfo,
    next_ptr: *const T,
) {
    unsafe {
...
pub fn check_extension_support(
    ...
    log::info!(
        "Require {} Extensions: {} / {} availables.",
        extension_type,
        require_extensions.len(),
        require_extensions.len()
    );
...
pub fn get_max_usable_sample_count(
    ...
    .iter()
    .filter(|&&x| sample_count_limit.contains(x))
    .next()
    .unwrap();
...
pub fn create_vk_instance(
    ...
    log::info!(
        "    engine version: {}.{}.{}",
        vk::api_version_major(constants::ENGINE_VERSION),
        vk::api_version_minor(constants::ENGINE_VERSION),
        vk::api_version_patch(constants::ENGINE_VERSION)
    );
    unsafe {
        log::info!(
            "    require vulkan api version: {}.{}.{}",
            vk::api_version_major(constants::VULKAN_API_VERSION),
            vk::api_version_minor(constants::VULKAN_API_VERSION),
            vk::api_version_patch(constants::VULKAN_API_VERSION)
        );
    }
