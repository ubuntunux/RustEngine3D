...
data ColladaNode
    ...
    | ColladaNode
        { _is_valid_node :: Bool
        , _node_name :: Text.Text
        , _node_id :: Text.Text
        , _node_type :: Text.Text
        , _node_matrix :: Mat44f
        , _parent_node :: ColladaNode
        , _child_nodes :: [ColladaNode]
        }
    deriving (Eq, Show)

get_xml_tree :: IOSArrow XmlTree XmlTree -> Int -> IO [XmlTree]
get_xml_tree xml_node index = runX xml_node

-- [NTree (XTag "author" [NTree (XAttr "x") [NTree (XText "99") []]]) [NTree (XText "Blender User") []],NTree (XTag "author" [NTree (XAttr "z") [NTree (XText "22") []],NTree (XAttr "y") [NTree (XText "11") []]]) [NTree (XText "Blender User2") []],NTree (XTag "author" []) [],NTree (XTag "author" []) []]

get_xml_tree_tag :: XmlTree -> Text.Text
get_xml_tree_tag (NTree (XTag tag _) _) = Text.replace "\"" "" $ Text.pack (show tag)
get_xml_tree_tag _ = Text.empty

get_xml_tree_attrib :: XmlTree -> Text.Text -> Text.Text
get_xml_tree_attrib (NTree (XTag _ attribs) _) attrib_name =
    get_xml_tree_attrib' attrib_name attribs
    where
        get_xml_tree_attrib' :: Text.Text -> [XmlTree] -> Text.Text
        get_xml_tree_attrib' search_attrib_name ((NTree (XAttr attrib) attrib_values):attribs) =
            let attrib_string = Text.replace "\"" "" $ Text.pack (show attrib)
            in
                if attrib_string == search_attrib_name then
                    case attrib_values of
                        [] -> Text.empty
                        otherwise -> let (NTree (XText value) _) = attrib_values !! 0 in Text.pack value
                else
                    get_xml_tree_attrib' search_attrib_name attribs
        get_xml_tree_attrib' search_attrib_name ((NTree _ attrib_values):attribs) = Text.empty
        get_xml_tree_attrib' search_attrib_name [] = Text.empty
get_xml_tree_attrib _ _ = Text.empty

get_xml_tree_text :: XmlTree -> Text.Text
get_xml_tree_text (NTree _ ((NTree (XText text) []):_)) = Text.pack text
get_xml_tree_text (NTree _ _) = Text.empty

--

get_xml_child :: IOSArrow XmlTree XmlTree -> Text.Text -> IOSArrow XmlTree XmlTree
get_xml_child xml_node child_path =
    let splited_path = map Text.unpack (Text.splitOn "/" child_path)
    in foldl (\acc x -> acc /> hasName x) xml_node splited_path

get_xml_child_deep :: IOSArrow XmlTree XmlTree -> Text.Text -> IOSArrow XmlTree XmlTree
get_xml_child_deep xml_node child_path =
    let splited_path = map Text.unpack (Text.splitOn "/" child_path)
    in get_xml_child_deep' xml_node splited_path
    where
        get_xml_child_deep' parent_node [] = parent_node
        get_xml_child_deep' parent_node (x:[]) = parent_node //> hasName x
        get_xml_child_deep' parent_node (x:xs) = get_xml_child_deep' (parent_node //> (hasName x)) xs

get_xml_attrib_value' :: IOSArrow XmlTree XmlTree -> Text.Text -> Int -> Text.Text -> IO Text.Text
get_xml_attrib_value' xml_node attribute_name child_index default_value = do
    attribute <- runX $ xml_node >>> getAttrValue (Text.unpack attribute_name)
    return $ if child_index < length attribute then Text.pack (attribute !! child_index) else default_value

get_xml_attrib_value :: IOSArrow XmlTree XmlTree -> Text.Text -> IO Text.Text
get_xml_attrib_value xml_node attribute_name = get_xml_attrib_value' xml_node attribute_name 0 Text.empty

get_xml_text' :: IOSArrow XmlTree XmlTree -> Int -> Text.Text -> IO Text.Text
get_xml_text' xml_node child_index default_value = do
    text <- runX $ xml_node /> getText
    return $ if child_index < length text then Text.pack (text !! child_index) else default_value

get_xml_text :: IOSArrow XmlTree XmlTree -> IO Text.Text
get_xml_text xml_node = get_xml_text' xml_node 0 Text.empty

get_xml_texts :: IOSArrow XmlTree XmlTree -> IO [Text.Text]
get_xml_texts xml_node = do
    text <- runX $ xml_node /> getText
    return $ map Text.pack text

show_xml_node :: IOSArrow XmlTree XmlTree -> IO ()
show_xml_node xml_node = do
    node <- runX xml_node
    print node
...
parsing_collada_nodes :: IOSArrow XmlTree XmlTree -> ColladaNode -> IO ColladaNode
parsing_collada_nodes xml_node parent_node = do
    node_name <- get_xml_attrib_value xml_node "name"
    node_id <- get_xml_attrib_value xml_node "id"
    node_type <- get_xml_attrib_value xml_node "type"

    instance_controller <- get_xml_attrib_value (get_xml_child xml_node "instance_controller") "url"
        >>= \url_string -> return $ if not (Text.null url_string) && '#' == (Text.head url_string) then (Text.tail url_string) else url_string

    instance_geometry <- get_xml_attrib_value (get_xml_child xml_node "instance_geometry") "url"
        >>= \url_string -> return $ if not (Text.null url_string) && '#' == (Text.head url_string) then (Text.tail url_string) else url_string

    parsing_matrix xml_node
...
--            self.children.append(child)
    ...
    let collada_node = ColladaNode
            { _is_valid_node = False
            , _node_name = Text.replace "." "_" node_name
            , _node_id = Text.replace "." "_" node_id
            , _node_type = node_type
            , _node_matrix = matrix4x4_indentity
            , _parent_node = parent_node
            , _child_nodes = []
            }
    return collada_node
    where
        parsing_matrix :: IOSArrow XmlTree XmlTree -> IO ()
        parsing_matrix xml_node = return ()
...
loadCollada filepath = do
    ...
    let xml_root = readString [withParseHTML yes, withWarnings no] contents
        xml_collada = get_xml_child xml_root "collada"
    collada_version <- get_xml_attrib_value xml_collada "version"
    author <- get_xml_text $ get_xml_child xml_collada "asset/contributor/author"
    authoring_tool <- get_xml_text $ get_xml_child xml_collada "asset/contributor/authoring_tool"
    created <- get_xml_text $ get_xml_child xml_collada "asset/created"
    modified <- get_xml_text $ get_xml_child xml_collada "asset/modified"
    unit_name <- get_xml_attrib_value (get_xml_child xml_collada "asset/unit") "name"
    unit_scale <- (read . Text.unpack::Text.Text -> Float) <$> get_xml_attrib_value (get_xml_child xml_collada "asset/unit") unit_name
    up_axis <- get_xml_text $ get_xml_child xml_collada "asset/up_axis"

    xs <- runX $ get_xml_child xml_collada "asset/contributor/author"
    print $ map (\x -> get_xml_tree_attrib x "z") xs
    print $ map (\x -> get_xml_tree_text x) xs

    print (collada_version, author, authoring_tool, created, modified, unit_name, unit_scale, up_axis)

    let xml_nodes = get_xml_child xml_collada "library_visual_scenes/visual_scene/node"
    node <- parsing_collada_nodes xml_nodes ColladaEmptyNode
    print node
    xs <- runX (get_xml_child xml_collada "asset/contributor/author")
    print xs
    print (get_xml_tree_tag $ xs !! 0)
    return []
