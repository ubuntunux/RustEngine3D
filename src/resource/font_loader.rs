...
pub fn get_font_data_create_info(
    ...
    for text in range_min..(range_max + 1) {
        ...
        let glyphs: Vec<PositionedGlyph> = font
            .layout(
                String::from_utf8(vec![text as u8]).unwrap().as_str(),
                scale,
        ...
        match (glyphs_first_bounding_box, glyphs_last_bounding_box) {
            (Some(glyphs_first_bounding_box), Some(glyphs_last_bounding_box)) => {
                ...
                glyphs_width = max(
                    glyphs_width,
                    ((max_x - min_x) as f32 + padding * 2.0) as u32,
                );
            }
            _ => {}
        }
    ...
    let mut image =
        DynamicImage::new_rgba8(glyphs_width * count_of_side, glyphs_height * count_of_side)
            .to_rgba8();
    ...
    if font::USE_DISTANCE_FIELD {
        ...
        let mut distance_field_image =
            DynamicImage::new_rgba8(image.width(), image.height()).to_rgba8();
        for y in 0..count_of_side {
            for x in 0..count_of_side {
                for py in 0..cell_height {
                    for px in 0..cell_width {
                        ...
                        let pos: Vector2<f32> =
                            Vector2::new((offset_x + px) as f32, (offset_y + py) as f32);
                        for sy in 0..cell_height {
                            for sx in 0..cell_width {
                                ...
                                if 0.5 < opacity {
                                    let src_pos: Vector2<f32> = Vector2::new(
                                        (offset_x + sx) as f32,
                                        (offset_y + sy) as f32,
                                    );
                        ...
                        distance_field_image.put_pixel(
                            offset_x + px,
                            offset_y + py,
                            Rgba([
                                colour.0,
                                colour.1,
                                colour.2,
                                255 - 255u8.min(((min_dist / max_dist).sqrt() * 255.0) as u8),
                            ]),
                        );
                    }
                }
