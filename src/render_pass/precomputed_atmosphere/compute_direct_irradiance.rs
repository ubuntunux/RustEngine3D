...
pub fn get_framebuffer_data_create_info(renderer_data: &RendererData) -> FramebufferDataCreateInfo {
    let render_target0 =
        renderer_data.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_DELTA_IRRADIANCE);
    let render_target1 =
        renderer_data.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_IRRADIANCE);
...
pub fn get_render_pass_data_create_info(renderer_data: &RendererData) -> RenderPassDataCreateInfo {
    ...
    let pipeline_data_create_info = PipelineDataCreateInfo {
        ...
        _pipeline_fragment_shader_file: PathBuf::from(
            "precomputed_atmosphere/compute_direct_irradiance.frag",
        ),
