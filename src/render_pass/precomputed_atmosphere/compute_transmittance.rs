...
use crate::scene::precomputed_atmosphere::DEFAULT_USE_COMBINED_TEXTURES;

pub fn get_framebuffer_data_create_info(renderer_data: &RendererData) -> FramebufferDataCreateInfo {
    let render_target =
        renderer_data.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_TRANSMITTANCE);
    let render_target_infos: [RenderTargetInfo; 1] = [RenderTargetInfo {
    ...
    let pipeline_data_create_infos = vec![
        PipelineDataCreateInfo {
            ...
            _pipeline_fragment_shader_file: PathBuf::from(
                "precomputed_atmosphere/compute_transmittance.frag",
            ),
        ...
        PipelineDataCreateInfo {
            ...
            _pipeline_fragment_shader_file: PathBuf::from(
                "precomputed_atmosphere/recompute_transmittance.frag",
            ),
            _pipeline_shader_defines: vec![format!(
                "COMBINED_SCATTERING_TEXTURES={:?}",
                if DEFAULT_USE_COMBINED_TEXTURES { 1 } else { 0 }
