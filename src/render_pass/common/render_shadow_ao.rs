...
pub fn get_render_pass_data_create_info(renderer_data: &RendererData) -> RenderPassDataCreateInfo {
    ...
    let subpass_dependencies = vec![vk::SubpassDependency {
        ...
        dst_subpass: 0,
        src_stage_mask: vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT // For GBuffer color writes (SceneNormal)
            | vk::PipelineStageFlags::EARLY_FRAGMENT_TESTS // For depth writes (SceneDepth, Shadow)
            | vk::PipelineStageFlags::LATE_FRAGMENT_TESTS, // For depth writes (SceneDepth, Shadow)
        src_access_mask: vk::AccessFlags::COLOR_ATTACHMENT_WRITE // For GBuffer color writes
            | vk::AccessFlags::DEPTH_STENCIL_ATTACHMENT_WRITE, // For depth writes
