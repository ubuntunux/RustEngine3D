...
impl<'a> EffectManager<'a> {
    ...
    pub fn clear_effects(&mut self) {
        unsafe {
            ...
            self._effect_render_group.clear();
            self._gpu_particle_static_constants =
                vec![GpuParticleStaticConstants::default(); MAX_EMITTER_COUNT as usize];
            self._gpu_particle_dynamic_constants =
                vec![GpuParticleDynamicConstants::default(); MAX_EMITTER_COUNT as usize];
    ...
    pub fn clear_gpu_particles(
        ...
        let thread_group_count =
            (dispatch_count + PROCESS_GPU_PARTICLE_WORK_GROUP_SIZE - 1) / PROCESS_GPU_PARTICLE_WORK_GROUP_SIZE;
        ...
        let pipeline_binding_data: &PipelineBindingData =
            material_instance_data.get_pipeline_binding_data("process_gpu_particle/update_gpu_particle");
    ...
    pub fn process_gpu_particles(
        ...
        {
            let gpu_particle_static_constants_buffer =
                renderer_data.get_shader_buffer_data(&ShaderBufferDataType::GpuParticleStaticConstants);
            let gpu_particle_dynamic_constants_buffer =
                renderer_data.get_shader_buffer_data(&ShaderBufferDataType::GpuParticleDynamicConstants);
            let gpu_particle_emitter_index_buffer =
                renderer_data.get_shader_buffer_data(&ShaderBufferDataType::GpuParticleEmitterIndexBuffer);
            let gpu_particle_count_buffer =
                renderer_data.get_shader_buffer_data(&ShaderBufferDataType::GpuParticleCountBuffer);
            let gpu_particle_update_buffer =
                renderer_data.get_shader_buffer_data(&ShaderBufferDataType::GpuParticleUpdateBuffer);
    ...
    pub fn render_effects(
        ...
        for emitter in self._effect_render_group.iter() {
            ...
            if ParticleGeometryType::Mesh == emitter_data._geometry_type {
                geometry_data = emitter_data._mesh_data.borrow().get_default_geometry_data().as_ptr();
            } else {
                assert!(
                    false,
                    "Not Implemented ParticleGeometryType: {:?}",
                    emitter_data._geometry_type
                );
            }
            assert_ne!(geometry_data, std::ptr::null(), "Unknown GeometryData");
