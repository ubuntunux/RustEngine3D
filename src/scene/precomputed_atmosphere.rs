...
fn cie_color_matching_function_table_value(wavelength: f32, column: i32) -> f32 {
    ...
    assert!(
        CIE_2_DEG_COLOR_MATCHING_FUNCTIONS[4 * row as usize] <= wavelength
            && wavelength <= CIE_2_DEG_COLOR_MATCHING_FUNCTIONS[4 * (row as usize + 1)]
    );

    u -= row as f32;
    CIE_2_DEG_COLOR_MATCHING_FUNCTIONS[(4 * row + column) as usize] * (1.0 - u)
        + CIE_2_DEG_COLOR_MATCHING_FUNCTIONS[(4 * (row + 1) + column) as usize] * u
}
...
impl DensityProfileLayer {
    pub fn create_density_profile_layer(
        width: f32,
        exp_term: f32,
        exp_scale: f32,
        linear_term: f32,
...
impl AtmosphereModel {
    ...
    fn shader_header_factory(&mut self, lambdas: &[f32; 3]) -> String {
        ...
        vec![
            ...
            format!(
                "{}, ",
                to_string(&self._mie_scattering, lambdas, self._length_unit_in_meters)
            ),
            format!(
                "{}, ",
                to_string(&self._mie_extinction, lambdas, self._length_unit_in_meters)
            ),
    ...
    fn precompute(
        ...
        renderer_context.render_material_instance(
            ...
            "precomputed_atmosphere/precomputed_atmosphere",
            if blend {
                "compute_direct_irradiance/default"
            } else {
                "compute_direct_irradiance/additive"
            },
            quad_geometry_data,
            None,
            None,
        ...
        for layer in 0..SCATTERING_TEXTURE_DEPTH as usize {
            ...
            renderer_context.render_material_instance(
                ...
                "precomputed_atmosphere/precomputed_atmosphere",
                if blend {
                    "compute_single_scattering/additive"
                } else {
                    "compute_single_scattering/default"
                },
                quad_geometry_data,
...
impl<'a> Atmosphere<'a> {
    pub fn create_atmosphere(is_render_atmosphere: bool) -> Self {
        ...
        Self {
            ...
            _luminance_type: luminance_type,
            _num_precomputed_wavelengths: if Luminance::PRECOMPUTED == luminance_type {
                15
            } else {
                3
            },
    ...
    pub fn prepare_framebuffer_and_descriptors(
        ...
        if false == USE_BAKED_PRECOMPUTED_ATMOSPHERE_TEXTURES {
            ...
            let compute_single_scattering_pipeline_binding_data =
                material_instance.get_pipeline_binding_data("compute_single_scattering/default");
            let compute_scattering_density_pipeline_binding_data =
            ...
            for layer in 0..SCATTERING_TEXTURE_DEPTH as u32 {
                ...
                let compute_single_scattering_rendertargets = vec![
                    ...
                    RenderTargetInfo {
                        _texture_data: &delta_mie_scattering,
                        _target_layer: layer,
                        _target_mip_level: 0,
                        _clear_value: None,
                    },
                    ...
                    RenderTargetInfo {
                        _texture_data: &optional_single_mie_scattering,
                        _target_layer: layer,
                        _target_mip_level: 0,
                        _clear_value: None,
                    },
