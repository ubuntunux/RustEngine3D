        ...
        for maybe_render_pass_pipeline_data in render_pass_pipeline_data_list.iter() {
            if maybe_render_pass_pipeline_data.is_some() {
                ...
                if check_push_consant_names.is_empty() {
                    check_push_consant_names = render_pass_pipeline_data
                        ._pipeline_data
                        .borrow()
                        ._push_constant_data_list
                        .iter()
                        .map(|push_constant_data| {
                            String::from(push_constant_data._push_constant.get_push_constant_name())
                        })
                        .collect();
                } else {
                    let push_constant_names: Vec<String> = render_pass_pipeline_data
                        ._pipeline_data
                        .borrow()
                        ._push_constant_data_list
                        .iter()
                        .map(|push_constant_data: &PipelinePushConstantData| {
                            String::from(push_constant_data._push_constant.get_push_constant_name())
                        })
                        .collect();
