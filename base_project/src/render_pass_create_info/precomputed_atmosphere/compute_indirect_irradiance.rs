...
pub fn get_framebuffer_data_create_info(renderer: &Renderer) -> FramebufferDataCreateInfo {
    let render_target0 = renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_DELTA_IRRADIANCE);
    let render_target1 = renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_IRRADIANCE);
